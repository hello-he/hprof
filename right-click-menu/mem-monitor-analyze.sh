#!/bin/bash
# mem-monitor hprof 分析脚本
# 此脚本会自动检测jar文件位置

# 获取脚本所在目录
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# jar文件路径（必须在脚本同一目录）
JAR_PATH="$SCRIPT_DIR/mem-monitor-1.0.0-all.jar"

# 检查jar文件是否存在
if [ ! -f "$JAR_PATH" ]; then
    zenity --error --text="错误：找不到 mem-monitor jar 文件\n\n请确保 jar 文件位于：\n$JAR_PATH" --title="内存泄露分析" 2>/dev/null || \
    echo "错误：找不到 mem-monitor jar 文件: $JAR_PATH"
    exit 1
fi
...
java -jar "$JAR_PATH" analyze -f "$1" -o "$BASE_REPORT_DIR" 2>&1 | tee /tmp/mem-monitor-analyze.log
ANALYZE_EXIT_CODE=$?
...
if [ $ANALYZE_EXIT_CODE -eq 0 ]; then
    ...
    zenity --error --text="分析失败！\n\n请查看日志: /tmp/mem-monitor-analyze.log" --title="内存泄露分析" 2>/dev/null || \
    echo "分析失败！请查看日志: /tmp/mem-monitor-analyze.log"
    cat /tmp/mem-monitor-analyze.log | tail -20
fi
