...
class BitmapExtractor {
    ...
    fun generateHtmlReport(result: ExtractionResult): String {
        ...
        sb.appendLine("            --text: #1e293b; --text-muted: #64748b; --accent: #be185d; --accent-hover: #9d174d;")
        ...
        sb.appendLine("        .header { background: linear-gradient(135deg, #be185d 0%, #831843 100%); color: white; padding: 32px 28px; }")
        sb.appendLine("        .header h1 { font-size: 1.6rem; font-weight: 700; margin-bottom: 8px; letter-spacing: -0.02em; }")
        sb.appendLine("        .header .subtitle { opacity: 0.9; font-size: 0.875rem; }")
        ...
        sb.appendLine("        .duplicate-group { background: var(--warn-bg); padding: 18px; border-radius: var(--radius-sm); margin-bottom: 18px; border-left: 4px solid var(--warn); }")
        sb.appendLine("        .duplicate-group .hash { font-family: var(--font-mono); color: var(--text-muted); font-size: 0.75rem; word-break: break-all; }")
        sb.appendLine("        .bitmap-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(100px, 1fr)); gap: 12px; margin-top: 16px; }")
        sb.appendLine("        .bitmap-item { background: white; border-radius: var(--radius-sm); overflow: hidden; box-shadow: var(--shadow); transition: transform 0.2s; }")
        sb.appendLine("        .bitmap-item:hover { transform: translateY(-2px); }")
        sb.appendLine("        .bitmap-item img { max-width: 100%; max-height: 80px; object-fit: contain; display: block; margin: 0 auto; }")
        sb.appendLine("        .bitmap-info { padding: 12px; font-size: 0.75rem; color: var(--text-muted); text-align: center; font-family: var(--font-mono); }")
        sb.appendLine("        .warning { background: var(--danger-bg); border-left: 4px solid var(--danger); padding: 16px; border-radius: var(--radius-sm); margin: 18px 0; }")
        sb.appendLine("        .warning .title { font-weight: 600; color: var(--danger); margin-bottom: 8px; }")
        sb.appendLine("        .gallery { display: grid; grid-template-columns: repeat(auto-fill, minmax(120px, 1fr)); gap: 12px; margin-top: 20px; }")
        sb.appendLine("        .gallery-item { position: relative; height: 120px; display: flex; align-items: center; justify-content: center; background: var(--bg-page); border-radius: var(--radius-sm); overflow: hidden; transition: box-shadow 0.2s; }")
        sb.appendLine("        .gallery-item:hover { box-shadow: var(--shadow); }")
        sb.appendLine("        .gallery-item img { max-width: 100%; max-height: 100px; object-fit: contain; border-radius: 4px; }")
        sb.appendLine("        .gallery-item .overlay { position: absolute; bottom: 0; left: 0; right: 0; background: rgba(0,0,0,0.7); color: white; padding: 6px; font-size: 0.6875rem; font-family: var(--font-mono); border-radius: 0 0 var(--radius-sm) var(--radius-sm); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }")
        sb.appendLine("        .info-note { background: var(--info-bg); padding: 16px; border-radius: var(--radius-sm); margin: 18px 0; font-size: 0.8125rem; border-left: 3px solid var(--info); }")
        sb.appendLine("        .info-note .title { font-weight: 600; color: var(--info); margin-bottom: 6px; }")
        sb.appendLine("        .info-note .link { color: var(--info); text-decoration: none; font-weight: 500; }")
        sb.appendLine("        .info-note .link:hover { text-decoration: underline; }")
        sb.appendLine("    </style>")
        sb.appendLine("</head>")
        sb.appendLine("<body>")
