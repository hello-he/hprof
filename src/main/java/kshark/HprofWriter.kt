...
class HprofWriter private constructor(
  ...
  private fun BufferedSink.write(record: HprofRecord) {
    when (record) {
      ...
      is StackFrameRecord -> {
        writeNonHeapRecord(HprofRecordTag.STACK_FRAME.tag) {
          writeId(record.id)
          writeId(record.methodNameStringId)
          writeId(record.methodSignatureStringId)
          writeId(record.sourceFileNameStringId)
          writeInt(record.classSerialNumber)
          writeInt(record.lineNumber)
        }
      }
      is GcRootRecord -> {
        with(workBuffer) {
